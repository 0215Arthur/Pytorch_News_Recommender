...
class MultiHeadSelfAttention(nn.Module):
  ...
  #  @torchsnooper.snoop()
    def forward(self, query, key, value, mask=None):
        ...
        x, attn = self.attention(query, key, value, head_nums=self.h, mask=mask)
...
class NewsEncoder(torch.nn.Module):
    ...
    def forward(self, data,attn_masks=None):
        ...
        title_embedded = self.word_embedding(title_ids) 
        attn_output = self.multi_head_self_attention(title_embedded,title_embedded,title_embedded)
        attn_output=self.dropout(attn_output)
        news_vector = self.additive_attention(attn_output)
         
         
        # news_vector=torch.stack(title_embeds_list).permute(1,0,2)
        ...
        #news_vector=self.dropout(title_vector)
...
class UserEncoder(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.multi_head_self_attention = MultiHeadSelfAttention(config.num_attention_heads, 
...
class Model(torch.nn.Module):
    ...
    def forward(self, batch):
       ...
       # candidate_title_ids=batch['candidate_titles'].to(self.config.device).permute(1,0,2)
        ...
        # b*E , 带mask epoch1:0.668206252072544
        user_vector = self.user_encoder(browsed_vector).unsqueeze(1)
