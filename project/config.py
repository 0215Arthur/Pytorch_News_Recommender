...
class Config(object):
    ...
    def __GLOBO__(self):
        ...
        self.news_nums=26011
        self.user_nums=76291
        self.article_embedding_pretrained =  'GLOBO/article_embed.npz'
    
    def __ADR__(self):
        self.cate_embed_size = 50
        ...
        self.negsample_size=4   # 负样本采样规模
        self.max_candidate_size=100  # 候选样本最大长度
        self.user_nums=189423
        self.news_nums=11075
        self.word_embedding_pretrained =  'ADR/all_word_embedding.npz'
        # self.entity_embedding_pretrained=  'MIND/entitiy_embeds.npz'
    ...
    def __nrms__(self):
        ...
        if self.dataset == "GLOBO":
            ...
            self.news_feature_size=1000
            self.query_vector_dim=400  # additive attn 向量维度
            self.query_vector_dim_large= 400 
            self.news_encoder_size= 600
            self.user_heads_num=5      # mind: 6
            self.title_heads_num=5
        else:
            self.news_feature_size=150
            self.query_vector_dim=100  # additive attn 向量维度
            self.query_vector_dim_large= 200 
            self.news_encoder_size= 50
            self.user_heads_num=5      # mind: 6
            self.title_heads_num=5
        self.num_heads_2=4         # 
        self.kernel_sizes = 3 # 
    ...
    def __tmgm__(self):
        ...
        if self.dataset == "GLOBO":
            ...
            self.time_thresh = [300, 3600, 12000]
            self.time_p = 0.5
            self.news_feature_size=150
            self.query_vector_dim=200  # additive attn 向量维度
            self.query_vector_dim_large= 200 
            self.news_encoder_size= 150
            self.user_heads_num=5      # mind: 6
            self.title_heads_num=2
